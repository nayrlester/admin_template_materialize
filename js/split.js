...
var global = this
  ...
  , Split = function (ids, options) {
    ...
    if (typeof options.snapOffset === 'undefined') options.snapOffset = 0
...
var draged=false;
    var startDragging = function (e) {
      ...
      , stopDragging = function () {
            ...
            if (self.dragging && draged && options.onDragEnd) {
                options.onDragEnd()
            }
            ...
            if(!draged){
              if (options.onHandleClick)options.onHandleClick(this);
            }
        }
      , drag = function (e) {
            draged=true;
            ...
            // Moved this to adjust so that all functions are limited this way
            // if (offset <=  this.aMin +  options.snapOffset ||offset<=this.aGutterSize) {
            //     offset = this.aMin<=this.aGutterSize ? this.aGutterSize : this.aMin
            // } else if (offset >= this.size - this.bMin  - options.snapOffset||offset>=this.size-this.bGutterSize) {
            //     offset = this.size - this.bMin <= this.size-this.bGutterSize? this.size - this.bMin:this.size-this.bGutterSize
            // }
    ...
    pair.updateOffset=updateOffset.bind(pair);
